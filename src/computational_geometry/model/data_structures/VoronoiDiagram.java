...
public class VoronoiDiagram extends HalfEdge {
	...
	public class VorEdge extends HalfEdge.Edge {
		...
		public void fill(Vert vert, Edge twin, Face face, Edge next) {
			if (!(twin instanceof VorEdge) ||
				!(face instanceof VorCell) ||
				!(next instanceof VorEdge)) {
				throw new IllegalArgumentException("VorEdge must be filled with VorEdges and VorCell");
			}
			super.fill(vert, twin, face, next);
		}
		
		public void fill(Segment s, Vert vert, VorEdge twin, VorCell cell, VorEdge next) {
			super.fill(vert, twin, face, next);
			this.s = s;
		}
	...
	public void addEdge(Edge e) {
		if (!(e instanceof VorEdge)) {
			throw new IllegalArgumentException("Can only add VorEdges to VoronoiDiagram.");
		}
		super.addEdge(e);
	}
